...
public class 튜플_Set미사용 {
    ...
    public static int[] solution(String s) {
        ...
        for (int i = 1; i < arr.length; i++) {
            ...
            int[] copy = Arrays.copyOf(answer, answer.length);

            int another = getNotEqualNum(copy, newArr);
            answer[i] = another;
        }
...
//        }
    ...
    public static int getNotEqualNum(int[] answer, int[] target) {
        answer = Arrays.stream(answer).filter(i -> i != 0).sorted().toArray();
        Arrays.sort(target);
        int mismatch = Arrays.mismatch(target,answer);
